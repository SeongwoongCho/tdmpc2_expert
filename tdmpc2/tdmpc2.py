...
class TDMPC2:
    ...
    def load(self, fp):
        ...
        if str(self.cfg.task) in ["cartpole-balance", "cartpole-swingup", "cartpole-swingup-sparse", "walker-walk-backwards"]:
            for key1, key2 in zip([key for key in self.model.state_dict().keys() if key != "_dynamics.1.weight" and key != "_dynamics.1.bias"], [key for key in state_dict["model"].keys() if key != "_dynamics.1.weight" and key != "_dynamics.1.bias"]):
                print(key1, key2)
                state_dict["model"][key1] = state_dict["model"].pop(key2)
